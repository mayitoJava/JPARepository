...
import AVilchis.ProgramacionNCapasNoviembre25.Service.UserDetailsJPAService;
...
public class SpringSecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        http.authorizeHttpRequests( config -> config.requestMatchers("/usuario/**")
            .hasAnyRole("Ingeniero")
            .anyRequest().authenticated()
        ).formLogin(form -> form.defaultSuccessUrl("/usuario", true).loginPage("/login").permitAll()
